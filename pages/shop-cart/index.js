//index.js
var goodsDetailList = require('../../utils/goodsDetailList.js')
var app = getApp()
Page({
  data: {
    goodsList:{
    ...
    toPayOrder:function(){
	  ...
	  /*  for(var j=0;j<goodsDetailList.goodsDetailList.data.length;j++){
	        	...
	        	if(carShopBean.goodsId==goodsDetailTmp.basicInfo.id){
	             doneNumber++;
	              if (goodsDetailTmp.properties) {
	                wx.showModal({
	                  title: '提示',
	                  content: goodsDetailTmp.basicInfo.name + ' 商品已失效，请重新购买',
	                  showCancel:false
	                })
	                isFail = true;
	                wx.hideLoading();
	              ...
	              if (goodsDetailTmp.basicInfo.stores < carShopBean.number) {
	                wx.showModal({
	                  title: '提示',
	                  content: goodsDetailTmp.basicInfo.name + ' 库存不足，请重新购买',
	                  showCancel:false
	                })
	                isFail = true;
	                wx.hideLoading();
	              ...
	              if (goodsDetailTmp.basicInfo.minPrice != carShopBean.price) {
	                wx.showModal({
	                  title: '提示',
	                  content: goodsDetailTmp.basicInfo.name + ' 价格有调整，请重新购买',
	                  showCancel:false
	                })
	                isFail = true;
	                wx.hideLoading();
